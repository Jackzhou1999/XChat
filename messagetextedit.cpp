...
QStringList MessageTextEdit::getUrl(QString text)
{
    ...
    foreach (QString url, list) {
        if(!url.isEmpty()){
            ...
            qDebug() << "IN MessageTextEdit::getUrl:" << str;
            if(str.size()>=2)
...
QPixmap MessageTextEdit::getFileIconPixmap(const QString &url)
{
    ...
    QFileInfo fileinfo(QUrl::fromPercentEncoding(url.toUtf8()));
    QIcon icon = provder.icon(fileinfo);

    QString strFileSize = getFileSize(fileinfo.size());

    qDebug() << strFileSize << " " << fileinfo.size();
    QFont font(QString("ubuntu"),10,QFont::Normal,false);
    QFontMetrics fontMetrics(font);

    QString filename = QUrl::fromPercentEncoding(fileinfo.fileName().toUtf8());

    QSize textSize = fontMetrics.size(Qt::TextSingleLine, filename);
    ...
    painter.drawPixmap(rect, icon.pixmap(50,50));
    painter.setPen(Qt::black);
    // 文件名称
    QRect rectText(50+10, 3, textSize.width()+40, textSize.height());
    painter.drawText(rectText, filename);
    // 文件大小
    QRect rectFile(50+10, textSize.height()+5, FileSize.width()+20, FileSize.height());
