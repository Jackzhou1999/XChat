...
BubbleFrame::BubbleFrame(ChatRole role, QWidget *parent):QFrame(parent),m_role(role)
{
    m_pHLayout = new QHBoxLayout();
    if(m_role == ChatRole::Self){
        m_pHLayout->setContentsMargins(3,3,3+WIDTH_SANJIAO,3);
    }else{
        m_pHLayout->setContentsMargins(3+WIDTH_SANJIAO,3,3,3);
    }
