    ...
    ui(new Ui::ApplyFriend),_label_point(2,6)
{
    ...
    QFont font;
    font.setPointSize(9); // 设置字体大小为16
    ui->lb_ed->setFont(font);
    ui->lb_ed->setPlaceholderText("搜索、添加标签");

    ui->lb_ed->move(2, 2);
    ui->lb_ed->setFixedHeight(20);
    ui->lb_ed->setMaxLength(20);
    ...
    InitTipLbs();
    //链接输入标签回车事件
    connect(ui->lb_ed, &CustomLineEdit::returnPressed, this, &ApplyFriend::SlotLabelEnter);
    connect(ui->lb_ed, &CustomLineEdit::textChanged, this, &ApplyFriend::SlotLabelTextChange);
    connect(ui->lb_ed, &CustomLineEdit::editingFinished, this, &ApplyFriend::SlotLabelEditFinished);
...
void ApplyFriend::InitTipLbs()
{
    ...
    for(int i = 0; i < _tip_data.size(); i++){
        ...
        auto* lb = new ClickLabelPlus(ui->lb_list);
        lb->SetState("normal", "hover", "pressed", "selected_normal",
                     "selected_hover", "selected_pressed");
        lb->setObjectName("tipslb");
        lb->setText(_tip_data[i]);
        connect(lb, &ClickLabelPlus::clicked, this, &ApplyFriend::SlotChangeFriendLabelByTip);
...
void ApplyFriend::AddTipLbs(ClickLabelPlus* lb, QPoint cur_point, QPoint& next_point, int text_width, int text_height)
{
    lb->move(cur_point);
    lb->show();
...
void ApplyFriend::SetSearchInfo(std::shared_ptr<SearchInfo> si)
{
    ...
    auto applyname = "jackzhou";

    // auto bakname = si->_name;
    auto bakname = "jackzhou";
